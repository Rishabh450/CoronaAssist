...
public class NgoActivity extends AppCompatActivity {
    String ngoname;
    List<NgoActivityModel> list = new ArrayList<>();
                ...
                for (int l = 0; l < data.size(); l++) {
                    ...
                    if (response.body().getId().get(l).getName().equals(ngoname)) {
                        ...
                        if (response.body().getId().get(l).getActivities() != null) {
                            ...
                            List<NgoActivityModel> lister = response.body().getId().get(l).getActivities();
                            NgoActivityModel ngo = lister.get(0);
                            int i = 0;
                            if (ngo != null) {
                                for (; i < lister.size(); i++) {
                                    if (lister.get(i).lat == lat && lister.get(i).lon == lon) {
                                        ngo = lister.get(i);
